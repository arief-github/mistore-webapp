              ...
              <table class="table table-bordered">
                <client-only>
                  ...
                  <tr>
                    ...
                    <td>:</td>
                    <td>
                      <button v-if="invoice.status == 'pending'" @click="payment(invoice.snap_token)"
                              class="btn btn-info">BAYAR SEKARANG
                      </button>
                      <button v-else-if="invoice.status == 'success'"
                              class="btn btn-success"><i class="fa fa-checkcircle"></i> {{ invoice.status }}
                      </button>
                      <button
                        v-else-if="invoice.status =='expired'"
                        class="btn btn-warning-2"><i  class="fa fa exclamation-triangle"></i> {{ invoice.status }}
                      </button>
                      <button
                        v-else-if="invoice.status == 'failed'" class="btn btn-danger"><i class="fa fa-timescircle"></i> {{ invoice.status }}
                      </button>
                    </td>
                  </tr>
                </client-only>
          ...
          <div class="card border-0 rounded shadow-sm border-top-orange mt-4">
            <div class="card-header">
              <span class="font-weight-bold">
                <i class="fa fa-shopping-cart"></i> DETAIL ITEMS
              </span>
            </div>
            <div class="card-body">
                    ...
                    <tr v-for="order in invoice.orders" :key="order.id" style="background: #edf2f7;">
                      <td class="b-none" width="25%">
                        <div class="wrapper-image-cart">
                          <img :src="order.product.image" alt="order product image" style="width: 100%; border-radius: .5rem">
                        </div>
                      </td>
