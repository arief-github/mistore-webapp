            ...
            <div class="card-body">
              ...
              <div class="form-group">
                <div class="input-group mb-3">
                  <input v-model="search" type="text" class="form-control" placeholder="Cari berdasarkan no. invoice">
              ...
              <b-table striped bordered hover :items="invoices.data" :fields="fields" show-empty>
                ...
                <template v-slot:cell(status)="row">
                  <button v-if="row.item.status === 'pending'" class="btn btn-sm btn-primary">
                    <i class="fa fa-circle-notch fa-spin"></i> {{ row.item.status }}
                  </button>
                  <button v-if="row.item.status === 'success'" class="btn btn-sm btn-success">
                    <i class="fa fa-check-circle"></i> {{ row.item.status }}
                  </button>
                  <button v-if="row.item.status === 'expired'" class="btn btn-sm btn-warning">
                    <i class="fa fa-exclamation-triangle"></i> {{ row.item.status }}
                  </button>
                  <button v-if="row.item.status === 'failed'" class="btn btn-sm btn-danger">
              ...
              <b-pagination align="right" :value="invoices.current_page" :total-rows="invoices.total" :per-page="invoices.per_page" @change="changePage" aria-controls="my-table">
              </b-pagination>
            </div>
          </div>
        </div>
